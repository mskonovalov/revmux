...
and `cursor-agent --print` subprocesses, then returns findings on stdout as JSON or markdown.
...
alone for `claude-only`, codex alone for `codex-only`, cursor-agent alone for `cursor-only`. `preflight.sh`
in the shipped skill answers it for any profile and any invocation.

`ANTHROPIC_API_KEY` is stripped from the child environment by default so `claude` uses interactive
subscription auth; pass `--preserve-anthropic-api-key` if you authenticate by key. `CURSOR_API_KEY` is
left in place: it is how a headless `cursor-agent` authenticates when there is no login session.
...
codex and cursor-agent mix inside one review.
...
| `cursor-only` | the same splits on cursor-agent, across Composer 2.5, Grok 4.6, Grok 4.5 and GPT-5.6 Sol |
...
**The nine are starting points, not the menu.** A profile is a file under `prompts/profiles/`, so dropping
