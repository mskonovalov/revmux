...
func TestModel_key_tabs(t *testing.T) {
	tests := []struct {
		...
		{"a digit selects a pane directly", []string{"2"}, 2},
		{"zero returns to the combined view", []string{"2", "0"}, 0},
		{"a digit past the roster is ignored", []string{"1", "7"}, 1},
		{"an unbound key changes nothing", []string{"1", "z"}, 1},
	}
...
func TestModel_key_scroll(t *testing.T) {
	...
	t.Run("switching panes opens the new one at its newest line", func(t *testing.T) {
		m := feed(t, filled(t, 20), press("1"), press("g"))
	...
	t.Run("a pane with nothing in it does not scroll", func(t *testing.T) {
		m := feed(t, New(ModelConfig{Roster: roster()}), press("1"), press("up"))
		assert.Equal(t, 0, m.view.scroll)
	})
}
