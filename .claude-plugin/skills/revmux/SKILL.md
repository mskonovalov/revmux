...
case where "branch diff" and "just my uncommitted changes" are both plausible. Use AskUserQuestion
rather than prose, here and at the headless-versus-overlay choice in Step 4.
...
```bash
${CLAUDE_SKILL_DIR}/scripts/launch-revmux.sh --task <id> --run <name> [any other revmux flag]
```
...
```bash
${CLAUDE_SKILL_DIR}/scripts/launch-revmux.sh --task pr-123 --run round-1 > /tmp/revmux-pr-123.json
```
...
→ launch-revmux.sh --task tui-rework --run round-1 > /tmp/…json   (background)
